...
`plume-move-clock-scope` is centered on chess and game engines work in Elixir. The technical target is: Build an Elixir toolkit that studies clock behavior through seeded input scenarios, with deterministic summary checks and no production deployment claims.
